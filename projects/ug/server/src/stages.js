...
export class Stages {
  constructor(db, ledger, opts = {}) { this.db = db; this.ledger = ledger; this.sandbox = !!opts.sandbox; }
  ...
  create(userId, body) {
    ...
    this.db.prepare('insert into stages (id, route, day, win, cutoff, pickup, cap, min_fill, guarantee, status, created_by, created_at, circles) values (?,?,?,?,?,?,?,?,?,?,?,?,?)')
      .run(s.id, s.route, s.day, s.win, s.cutoff, s.pickup, s.cap, s.min_fill, s.guarantee, s.status, s.created_by, s.created_at, s.circles);
  ...
  queue(userId, stageId) {
    ...
    const dup = this.db.prepare(`select id from intents where stage_id = ? and user_id = ? and state not in ('refunded')`).get(stageId, userId);
  ...
  hold(intentId, { flwId = null, amount } = {}) {
    const it = this.db.prepare('select * from intents where id = ?').get(intentId); if (!it) throw httpError(404, 'no such intent');
    if (it.state !== 'queued') return this.intent(intentId);
    ...
    this.db.prepare(`update intents set state = 'held', flw_id = ?, updated_at = ? where id = ?`).run(flwId, now(), it.id);
    this.maybeManufacture(it.stage_id);
  ...
  // Cut-off sweep: a filling stage past its cut-off below minimum fill refunds everyone (airport stages carry the guarantee and stay open).
  sweepCutoffs(cutoffMs = 90 * 60 * 1000) { /* schedules are day/window strings in v1.2; the sweep is wired when stages carry timestamps */ return 0; }
}
